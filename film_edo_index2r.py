...
def preprocess():
    ...
    DB_dict_ids = compute_ids_for_DB_or_YAGO(DB_dict,"DB_Film_ids")
    YAGO_dict_ids = compute_ids_for_DB_or_YAGO(YAGO_dict,"YAGO_Film_ids")

    with open("DB_Film_triples","w") as input_file:
        json.dump(DB_dict_ids,input_file)
    
    with open("YAGO_Film_triples","w") as input_file:
